...
def load_data():
    ...
    # Clean data: Fill NaNs in important columns with 0 for display
    cols_to_fix = ['total_cases', 'new_cases', 'total_deaths', 'new_deaths', 'total_vaccinations', 'people_fully_vaccinated_per_hundred', 'positive_rate']
...
selected_countries = st.sidebar.multiselect(
    ...
    options=locations,
    default=["World", "India", "United States"]
)
...
# Latest Global Stats (for metrics) - Handle missing latest day data
latest_date = df['date'].max()
global_df = df[df['location'] == 'World'].sort_values('date')
global_data = global_df.iloc[-1]
# If latest global record is too empty, take the previous day
if global_data['total_cases'] == 0:
    global_data = global_df.iloc[-2]
...
if os.path.exists("banner.png"):
    ...
    st.image("https://images.unsplash.com/photo-1584118624012-df45f49c67e7?q=80&w=2000&auto=format&fit=crop", width='stretch')
